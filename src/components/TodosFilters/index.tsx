...
function TodoFilters(props: ITodoFiltersProps) {
  ...
  return (
    <div className="collapse-items__menu">
      ...
      {currentCategoryTodo === todosCategory[1] && (
        <span>{activeCount} items left</span>
      )}
      {currentCategoryTodo === todosCategory[2] && (
        <span>{completedCount} items completed</span>
      )}
      <div className="collapse-items__todo-filters">
        <button
          onClick={() => changeTodosCategory(todosCategory[0])}
          className={
            currentCategoryTodo === todosCategory[0]
        ...
        <button
          onClick={() => changeTodosCategory(todosCategory[1])}
          className={
            currentCategoryTodo === todosCategory[1]
              ? "collapse-items__filter-btn--active"
              : "collapse-items__filter-btn"
        ...
        <button
          onClick={() => changeTodosCategory(todosCategory[2])}
          className={
            currentCategoryTodo === todosCategory[2]
              ? "collapse-items__filter-btn--active"
              : "collapse-items__filter-btn"
